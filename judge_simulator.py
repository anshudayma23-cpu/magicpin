...
LLM_API_KEY = os.getenv("GROQ_API_KEY", "") # DO NOT HARDCODE KEYS HERE
...
class BotClient:
    ...
    def tick(self, triggers):
        return self._request("POST", "/v1/tick", timeout=300, body_dict={"now": datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), "available_triggers": triggers})
...
class JudgeSimulator:
    ...
    def _phase2_short(self) -> bool:
        ...
        trigs = list(self.dataset.triggers.keys())[:25]
        for tid in trigs:
    ...
    def _score_and_display(self, action: Dict, verbose: bool = True):
        ...
        import time
        time.sleep(15) # Throttle to avoid LLM rate limits
...
def main():
    import time
    time.sleep(30) # Initial cooldown to reset rate limits
